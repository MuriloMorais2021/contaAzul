...
<form method="POST">
    ...
    <select name="address_state" id="address_state" onchange="changeState(this)">
        <?php foreach ($states_list as $states) : ?>
            <option value="<?= $states['UF']; ?>" <?= $states['UF'] == $client_info['address_state'] ? 'selected' : ''; ?>><?= $states['UF']; ?></option>
        <?php endforeach; ?>
    </select> <br><br>

    <label for="address_city">Cidade</label><br>
    <select name="address_city" id="address_city">
        <?php foreach ($cities_list as $cities) : ?>
            <option value="<?= $cities['codigoMunicipio']; ?>" <?= $cities['codigoMunicipio'] == $client_info['address_citycode'] ? 'selected' : ''; ?>><?= $cities['Nome']; ?></option>
        <?php endforeach; ?>
    </select> <br><br>
